...
impl Keyboard {
    pub fn new(
        ...
        thread::spawn(move || {
            // A dropped link (keyboard sleep, BLE/USB disconnect, host sleep) makes
            // `hid_read` error repeatedly. Mark the connection as dead after a few
            // consecutive errors to trigger auto-reconnect.
