...
pub fn keycode_to_layout_key(keycode: &Keycode) -> LayoutKey {
    ...
    if raw >> 16 == 0x0007 {
        ...
        // both `tap` and `shifted` there. Space, Enter, etc. would otherwise
        // pick up a real but useless character from the OS (including
        // literal control chars), so they were deliberately left `None`.
        if key.shifted.is_some() {
            ...
            if let Some(base) = &os_base {
                // A layout can put a real letter on a US-symbol slot (the
                ...
                // there Shift produces a different, useful character, so
                // keep the normal Base+Shifted stack.
