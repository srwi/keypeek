use keypeek_core::modifier_symbols::*;
use keypeek_core::{
    KeycodeKind, Label, LayoutKey, HELD_MOD_RALT, HELD_MOD_SHIFT, PLAIN_ALT_MOD_MASK,
};
...
pub(crate) fn hid_keyboard_key_static(usage_id: u16) -> Option<LayoutKey> {
    match usage_id {
        ...
        0xE1 => Some(modifier_key(&MOD_SHIFT, HELD_MOD_SHIFT)),
        0xE2 => Some(modifier_key(&MOD_ALT, PLAIN_ALT_MOD_MASK)),
        0xE3 => Some(modifier_key(&MOD_GUI, 0)),
        0xE4 => Some(modifier_key(&MOD_CTRL, 0)),
        0xE5 => Some(modifier_key(&MOD_SHIFT, HELD_MOD_SHIFT)),
        // RAlt is the layout's Level-3 shift, on layouts that define one.
        0xE6 => Some(modifier_key(&MOD_ALT, HELD_MOD_RALT)),
