...
mod tests {
    ...
    fn round_trip_raw_hex() {
        struct MockHexProfile;
        impl crate::key_presenter::KeyPresenter for MockHexProfile {
            fn present_key(
                &self,
                _spec: &KeySpec,
                _layer_names: &[String],
        ...
        impl EditorProfile for MockHexProfile {
            fn presenter(&self) -> &dyn crate::key_presenter::KeyPresenter {
                self
            }
            ...
            fn sidebar_sections(
                &self,
            ) -> &[crate::keymap_editor::profile::SidebarSection<EditorSection>] {
                &[]
            }
            fn tap_categories(&self) -> &'static [crate::keymap_editor::candidate::CandidateGroup] {
                &[]
            }
            fn section_groups(
                &self,
                _section: EditorSection,
            ) -> &'static [crate::keymap_editor::candidate::CandidateGroup] {
                &[]
            }
            fn layer_groups(
                &self,
                ...
                _layer_names: &[String],
                _tap: LayerTapTarget,
            ) -> Vec<crate::keymap_editor::candidate::CandidateGroup> {
                vec![]
            }
            fn parse_raw_keycode(&self, raw: &str) -> Option<KeySpec> {
                (raw == "0004").then_some(KeySpec::KeyPress {
                    key: HidKey::keyboard(0x04),
        ...
        struct MockNoHexProfile;
        impl crate::key_presenter::KeyPresenter for MockNoHexProfile {
            fn present_key(
                &self,
                _spec: &KeySpec,
                _layer_names: &[String],
        ...
        impl EditorProfile for MockNoHexProfile {
            fn presenter(&self) -> &dyn crate::key_presenter::KeyPresenter {
                self
            }
            fn name(&self) -> &'static str {
        ...
        assert_eq!(draft.staged(), None);
        assert_eq!(
            draft.staged_for(&MockHexProfile),
            Some(KeySpec::KeyPress {
                key: HidKey::keyboard(0x04),
                modifiers: Default::default(),
            })
        );
        assert_eq!(draft.staged_for(&MockNoHexProfile), None);
    }
